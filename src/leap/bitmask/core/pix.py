...
def start_pixelated_user_agent(userid, soledad, keymanager):
    ...
    services_factory = SingleUserServicesFactory(UserAgentMode(is_single_user=True))
...
class LeapSessionAdapter(object):
    ...
    def __init__(self, userid, soledad, keymanager):
        ...
        smtp_client_cert = os.path.expanduser(
            '~/.config/leap/providers/mail.bitmask.net/keys/client/smtp_test_mail_23213.pem')
...
def _start_in_single_user_mode(leap_session, config, resource, services_factory):
...
def start_user_agent_in_single_user_mode(root_resource, services_factory, leap_home, leap_session):
    log.msg('Bootstrap done, loading services for user %s' % leap_session.userid)
